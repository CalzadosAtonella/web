document.addEventListener('DOMContentLoaded', function () {
    ...
    function actualizarCarrito() {
        ...
        carrito.forEach(producto => {
            ...
            li.textContent = `${producto.nombre} - S/${producto.precio.toLocaleString()}`;
    ...
    botonWhatsApp.addEventListener('click', () => {
        const mensaje = carrito.map(producto => `${producto.nombre} - S/${producto.precio.toLocaleString()}`).join('%0A');
        const total = carrito.reduce((sum, producto) => sum + producto.precio, 0);
        const url = `https://w.app/eyvubh?text=Hola, quiero hacer este pedido:%0A${mensaje}%0ATotal: S/${total.toLocaleString()}`;
